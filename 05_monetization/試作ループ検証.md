...
> 案を 10〜20 件追加 → 3 件選定 → 1 件試作（最低）→ テスト → candidate / hold / reject 判断 を一巡する。
...
```mermaid
flowchart LR
  Add["14案追加<br/>(APIなし優先)"] --> Pick["3案選定<br/>(N-01/N-03/N-04)"]
  Pick --> Proto["1案試作<br/>token-speed-tool"]
  Proto --> Test["静的テスト<br/>(構文/計算/JSON)"]
  Test --> Judge{"candidate化?"}
  Judge -->|"OK"| Yes["candidate化<br/>(scenarios候補)"]
  Judge -->|"次"| Next["N-03/N-04 次サイクル"]
```

> 用語注: 試作ループ = 案 → 試作 → テスト → レビュー → 判断 の一巡 / APIなし = 外部 LLM/SaaS API を呼ばない / candidate = ChatGPT 承認前の有力候補 / N-XX = 本サイクル追加案 ID
