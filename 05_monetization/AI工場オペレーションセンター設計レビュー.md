...
Approval {
  ...
  epicId?: string
  title: string
  priority: 'critical' | 'high' | 'normal' | 'low'        // ★ G3
...
- 優先度バッジ 🔴critical / 🟠high / 🟡normal / ⚪low。critical を最上部に固定ソート。
...
- 操作（開始/停止/再開）は **Phase2**（実プロセス制御。MVPは表示のみ）。
...
> 方針: **「AIが止まらない運用基盤」を先に完成 → その後に自動実行制御**。各段は既存スキーマ非破壊。

| 順 | 項目 | 理由 | 依存 |
|---|---|---|---|
| 1 | **継続実行ポリシー確定**（§6 の停止条件リスト） | コード不要・即時。AIが止まる/止まらないの基準を先に固定 | なし |
| 2 | **Epic構造化**（`epics.json`・§3.1） | 情報分散で止まらない核。Goal明確＝継続実行の根拠 | 1 |
| 3 | **ToDo `source`**（§3.2） | AI生成ToDo/レビュー由来を追跡。ループ台帳化の基盤 | 2 |
| 4 | **Approval優先度＋選択式**（§3.3 / §4.2） | モバイル承認の核。承認を軽くしAIを止めない | 1,3 |
| 5 | **ヘルスダッシュボード**（§3.4 / §4.1） | 既存JSON＋getStagnation流用で派生表示のみ | 2,3,4 |
| 6 | **運用Decision Log**（§3.5） | 次回実行引き継ぎ。再質問を消す | 4 |

→ 1〜6 完了で「止まらない管制塔」が成立。**自動実行の開始/停止/再開（実プロセス制御）はこの後**。
...
### 7.1 MVP（運用基盤・API/課金なし・既存非破壊）
1. `/operations` ページ（縦1カラム・モバイル）
2. ヘルスダッシュボード（§3.4・既存JSON＋getStagnationから**派生表示のみ**）
3. `epics.json`（§3.1・Goal/Progress/Remaining/LatestRun/NextAction・手動更新可）
4. ToDo `source`/`sourceRunId`/`epicId` 追加（§3.2・既存はuser既定）
5. `approvals.json` ＋ 選択式承認カード（§3.3 / §4.2・優先度ソート・1タップ決定）
6. `operational-decisions.ndjson`（§3.5・次回実行引き継ぎ）
7. 継続実行ポリシー明文化（§6）

### 7.2 Phase2（自動実行制御）
- 自動実行の **開始/停止/再開ボタン**（実プロセス制御）
- 実行中/次予定の**リアルタイム表示**
...
- **優先順位**: ①ポリシー確定 ②Epic構造化 ③source ④選択式承認 ⑤ヘルス ⑥運用Decision Log → その後に自動実行制御。
- **確認したい点**:
  1. `/operations` 単一ページ集約 vs `/automation`分離、どちらが運用しやすいか。
  2. Approval優先度4段階（critical/high/normal/low）で粒度は十分か。
  3. 運用Decision Log を `decision-log.md`(収益化)と**別ファイル**にする分離方針で良いか。
  4. MVP範囲（自動実行制御を Phase2 送り）の線引きは妥当か。
