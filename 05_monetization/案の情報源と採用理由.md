...
> 詳細カード（15 件分）は正本 [[idea_trace]] にあります。本ページは**早見表 + 入口**。

> [!tip] 2026-05-29 追加: 市場調査（Research OS）由来の案
> Hermes の市場調査ログ（[[../06_research/daily-market-research/2026-05-27]] / [[../06_research/daily-market-research/2026-05-26]]）を news-app の **Research OS** が構造化 Topic として解析し、そこから派生した案（#11-15）を idea 段階で追加した。詳細は [[idea_trace]] §11-15。candidate / 承認はしていない。
...
| 11 | Indie App ASO Monitor | 市場調査(Research OS) ranking/official | Play 発見導線変化 / Scrape Lab 流用で APIなし成立 | idea（次サイクルで candidate 判断） |
| 12 | 学習スライド自動投稿メーカー | 市場調査(Research OS) ranking/social | AI 学習×縦動画導線 / 素材生成がボトルネック | idea |
| 13 | AI Study Battle | 市場調査(Research OS) social/reddit | 学習アプリ同質化 → 対戦・習慣化で差別化 | idea |
| 14 | MCP Profile Pack | 市場調査(Research OS) news/official | 文脈受け渡しの手間解消に需要 / APIなし成立 | idea |
| 15 | Agent Cost Logger | 市場調査(Research OS) official/news | 開発エージェント急増 × コスト可視化が空白 | idea |

---

## 📦 各案の詳細（カード形式・15 件）

各案の以下情報は [[idea_trace]] §1-§15 を参照（§11-15 = 市場調査 Research OS 由来）:
