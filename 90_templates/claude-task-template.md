...
- Claude による approved 化 / 承認確認 / 推奨を超える判断は禁止（Issue #17 / [[../03_prompts/Issue記述共通ルール]]）
- 停止条件以外でユーザーに「確認しますか？」を投げない（迷ったら仮説を文書化して進む）
...
## Issue 共通記述書式（Issue #17）

依頼本文・Issue 起票で**状態語を統一**する（[[../03_prompts/Issue記述共通ルール]] と同じ）:

| 状態 | 書き方 | 意味 |
|---|---|---|
| AI 推奨 | `推奨: <案>（承認ではない）` | Claude が比較し勧めるが決定ではない |
| 仮説 | `仮説: <前提> → このまま進行` | 不明点を仮置きして自走（停止しない） |
| 人間判断待ち | `人間判断待ち: <論点>` | approved / 公開 / 課金 等。Claude は触れない |
| ChatGPT 承認待ち | `chatgpt_pending` | 判断材料完備（[[../05_monetization/ChatGPT承認ゲート標準]]） |

「承認する」ではなく「推奨する / 比較する / 選定案を出す」と書く。完了条件は成果物ベース（比較表・推奨1件・計画 等）。
