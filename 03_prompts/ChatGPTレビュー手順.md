...
## 3. レビュー結果に必須の 8 項目 + 1 枚図サマリー（Issue #43）

レビューの**先頭**に「できるようになったこと」を 1 枚図サマリー（Mermaid 推奨）で示し、その後に 8 項目を明記する:

### 1 枚図サマリーの形式

レビュー冒頭に以下のいずれかを置く（必須・Issue #43）:

- **Mermaid 図**（推奨）: `flowchart LR` で「現在地 → 次の一手 → ゴール」の流れ
- Markdown の簡易フロー図（`A → B → C` 形式）
- 進捗バー付きサマリー（`[●●●○○] 60% Phase 3/5` 等）

ChatGPT が出力する Markdown は GitHub Web UI でも Obsidian でも Mermaid を描画できる。

```mermaid
flowchart LR
  Done[できるようになったこと] --> Now[現在地]
  Now --> Next[次の一手]
  Next --> Goal[ゴール]
```

### 標準レビュー構成（Issue #43 §標準レビュー構成）

```
1. GitHub 確認結果（§2 Step 1〜6 の要約）
2. できるようになったこと（1〜3 行）
3. 1 枚図サマリー（Mermaid / 必須）
4. どこで見れるか（パス / URL）
5. どう使うか（具体的な使い方）
6. 良い点（具体的に）
7. 気になる点（リスク / 抜け）
8. 次にやること（フォローアップ / 承認判断）
```

### 必須 8 項目（再掲・標準レビュー構成 4〜8 を含む）
