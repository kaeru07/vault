...
## 未レビュー
- [ ] [[2026-09-02_autorun-zero-apps-retrospective]]
  - createdAt: 2026-09-02 00:40
  - app: progress
  - project: AI工場の総括
  - priority: high
  - summary: 【失敗の総括】88日・1,480回の自動実行でアプリ完成0本・ストア公開0本・収益0円。TestFlight到達4本は全て人間指示が起点だった。原因=(1)工場の改良作業がアプリ制作と同じキューで競合し勝ち続けた（アプリ系ゴールdone11件は全て工場自身の機能追加でアプリ本体を進めたdoneは0件）(2)「自走完成の仕組みが出来た」を「アプリが完成した」と同じdoneで扱った（該当ゴールは7/9にdone=100だが実際のアプリ完成は伴わず）(3)空振り49%がcompletedとして積み上がった（8/24に是正）(4)生成試作2本が正本に接続されず7/20で停止 (5)提出・公開が人間依存のため「公開」をゴールに置くと定義上到達不能。自動実行は8/30に停止済み。progress側 runId 20260902-002949 を needs_human で登録、commit bce345c push済。確認観点=(1)原因分析の妥当性と見落とし (2)AI自律アプリ開発は現時点で現実的だったか (3)続けるなら何を変えるか/畳むべき理由 (4)空振り半分をどう受け止めるか。
  - result: 
...
- [ ] [[2026-08-22_sou-tiles-and-queue]]
  ...
  - project: 麻雀手牌解析AI ストア提出準備
  - priority: high
  - summary: 索子（ソーズ）牌を手書きSVGから生成スクリプト方式へ変更し、実物寄りの竹デザイン（円筒グラデーション+節2本+先端の締まり）に刷新。実物どおり5索=中央1本・7索=上1本・9索=上段3本を赤にし、赤ドラ5索は全本赤で通常5索と区別。1索(鳥)は対象外。あわせて残タスク4件を振り分け（Codex executor方針・950MB削除可否→今日の判断 / スクショ撮り直し・退役パス棚卸し→自動実行キュー）、退役パスを指していた goal-app-ny01-mahjong-analyzer を正本へ修正。検証=build/cap sync ios/26px実描画目視/回帰。commit b8e7db4 push済。追補: 索子刷新後の版でTestFlightビルド実行（全11ステップsuccess・App.ipa・Publishing完了 / runId 20260822-231900）、progressのiOSビルド一覧がNext既定キャッシュで古い応答を返す不具合も修正（d815b80）。さらにiPhoneでスクショ保存時に画面ごと遷移して戻れなくなる不具合を、共有シート優先の保存＋別タブの逃げ道で解消（765d55e / runId 20260823-011653）。確認観点=(1)実物の赤位置と配置の正しさ (2)赤ドラの表現と色依存判別のリスク (3)26px相当での可読性 (4)生成スクリプトを萬子・筒子へ広げる際の構造。
  - result: 
